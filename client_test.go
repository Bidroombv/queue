...
func TestClientStop(t *testing.T) {
	...
	t.Run("Close Consumer", func(t *testing.T) {
		...
		q, err := NewQueue().URL(url).Name(queueName).Consumer(3).Jobs(jobChannel).Connect()
	...
	t.Run("Close Publisher", func(t *testing.T) {
		...
		require.NoError(t, declareQueue(t, rabbitC, queueName))
		jobChannel := make(chan amqp.Delivery)
		q, err := NewQueue().URL(url).Name(queueName).Producer().Jobs(jobChannel).Connect()
		require.NoError(t, err)
...
func TestClientSingle(t *testing.T) {
	...
	// Consumer
	jobChannel := make(chan amqp.Delivery)
	qi, err := NewQueue().URL(url).Name(queueName).Consumer(1).Jobs(jobChannel).Connect()
	...
	qo, err := NewQueue().URL(url).Name(queueName).Producer().Jobs(jobChannel2).Connect()
	assert.NoError(t, err)
	defer qo.Close()
	pub := func(d amqp.Delivery) *amqp.Publishing {
...
func TestClientFast(t *testing.T) {
	...
	// Consumer
	qi, err := NewQueue().URL(url).Name(queueName).Consumer(1).Jobs(nil).Connect()
	require.NoError(t, err)
	defer qi.Close()
	...
	qo, err := NewQueue().URL(url).Name(queueName).Producer().Jobs(jobOutChannel).Connect()
	assert.NoError(t, err)
	defer qo.Close()
	pub := func(d amqp.Delivery) *amqp.Publishing {
		return &amqp.Publishing{
...
func TestClientFastWithExchange(t *testing.T) {
	...
	// Publisher
	jobOutChannel := make(chan amqp.Delivery)
	qo, err := NewExchange().URL(url).Name(exchangeName).Producer().Jobs(jobOutChannel).Connect()
	assert.NoError(t, err)
	defer qo.Close()
	pub := func(d amqp.Delivery) *amqp.Publishing {
		return &amqp.Publishing{
...
func TestHealthCheck(t *testing.T) {
	...
	q, err := NewQueue().URL(url).Name(queueName).Consumer(1).Jobs(nil).Connect()
	assert.NoError(t, err)
	defer q.Close()

	jobOutChannel := make(chan amqp.Delivery)
	ch, err := NewExchange().URL(url).Name(exchangeName).Producer().Jobs(jobOutChannel).Connect()
	assert.NoError(t, err)
	defer ch.Close()

	t.Run("should be health after connection setup", func(t *testing.T) {
		assertIsHealthy(t, q, true)
		assertIsHealthy(t, ch, true)
	})

	t.Run("stop the container", func(t *testing.T) {
